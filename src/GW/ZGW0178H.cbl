      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZGW0178H'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS REWRITE FILE('KSDSGW74')
                         FROM(KSDSGW74-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS STARTBR FILE('KSDSGW74')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSGW74')
                            INTO(KSDSGW74-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSGW74') END-EXEC.
      *----------------------------------------------------------------*
