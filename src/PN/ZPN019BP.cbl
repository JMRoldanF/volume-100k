      ******************************************************************
      * ZPN019BP - PENSION ADMINISTRATION                              *
      ******************************************************************
      *
      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZPN019BP'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS STARTBR FILE('KSDSPN95')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSPN95')
                            INTO(KSDSPN95-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSPN95') END-EXEC.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS REWRITE FILE('KSDSPN16')
                         FROM(KSDSPN16-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS DELETE FILE('KSDSPN16')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS STARTBR FILE('KSDSPN16')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSPN16')
                            INTO(KSDSPN16-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSPN16') END-EXEC.
      *----------------------------------------------------------------*
       AUDIT-MAKE-0016.
               PERFORM VARYING WS-IX FROM 1 BY 1
                           UNTIL WS-IX > WS-TABLE-COUNT
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS REWRITE FILE('KSDSPN95')
                         FROM(KSDSPN95-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
