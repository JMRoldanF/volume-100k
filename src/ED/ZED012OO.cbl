      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZED012OO'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS STARTBR FILE('KSDSED85')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSED85')
                            INTO(KSDSED85-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSED85') END-EXEC.
      *----------------------------------------------------------------*
       DERIVE-VALUE-0025.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0027.
               EXEC CICS DELETE FILE('KSDSED85')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
