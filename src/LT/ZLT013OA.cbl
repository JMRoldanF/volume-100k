      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZLT013OA'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSLT85')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSLT85')
                            INTO(KSDSLT85-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSLT85') END-EXEC.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS REWRITE FILE('KSDSLT85')
                         FROM(KSDSLT85-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
