      ******************************************************************
      * ZMA018TD - MARINE CARGO AND HULL                               *
      ******************************************************************
      *
      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMA018TD'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0036.
               EXEC CICS STARTBR FILE('KSDSMA19')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSMA19')
                            INTO(KSDSMA19-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSMA19') END-EXEC.
      *----------------------------------------------------------------*
