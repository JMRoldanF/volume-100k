      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMB0179Q'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZMB01D17-001.
               PERFORM CALL-ZMB01R9P-002.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS REWRITE FILE('KSDSMB77')
                         FROM(KSDSMB77-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS STARTBR FILE('KSDSMB77')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSMB77')
                            INTO(KSDSMB77-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSMB77') END-EXEC.
      *----------------------------------------------------------------*
       COMPUTE-MODEL-0016.
               EVALUATE TRUE
                  WHEN WS-PREMIUM-TOTAL < 999
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0027.
               EXEC CICS DELETE FILE('KSDSMB77')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
