      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMB013LU'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS DELETE FILE('KSDSMB47')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS STARTBR FILE('KSDSMB47')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSMB47')
                            INTO(KSDSMB47-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSMB47') END-EXEC.
      *----------------------------------------------------------------*
       WRITE-ERROR-MESSAGE.
