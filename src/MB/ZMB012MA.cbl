      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMB012MA'.
      ...
      * Called module names
       ...
       01  MOD-ZMB01LLM              PIC X(8) VALUE 'ZMB01LLM'.
      ...
      *----------------------------------------------------------------*
       CALL-ZMB01LLM-002.
               CALL 'ZMB01LLM' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS STARTBR FILE('KSDSMB23')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSMB23')
                            INTO(KSDSMB23-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSMB23') END-EXEC.
      *----------------------------------------------------------------*
