      ******************************************************************
      * ZNT0145Z - NOTIFICATION DISPATCH                               *
      ******************************************************************
      *
      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZNT0145Z'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS REWRITE FILE('KSDSNT38')
                         FROM(KSDSNT38-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS STARTBR FILE('KSDSNT38')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSNT38')
                            INTO(KSDSNT38-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSNT38') END-EXEC.
      *----------------------------------------------------------------*
       DERIVE-MODEL-0025.
               IF WS-KEY-CUSTOMER = ZERO
