      ******************************************************************
      * ZAR0104H - ARCHIVAL AND RETENTION                              *
      ******************************************************************
      *
      *  Generated volume-test source. Layer 2,
      *  type data_db2, domain ARCHIVE.
      ******************************************************************
      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZAR0104H'.
      ...
      * Called module names
       01  MOD-ZAR01D2Y              PIC X(8) VALUE 'ZAR01D2Y'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM NORMALISE-AGENT-CODE-0058.
               ...
               PERFORM COMPUTE-REG-NUMBER-0073.
               ...
               PERFORM COMPUTE-MODEL-0094.
               PERFORM CHECK-SUM-ASSURED-0095.
               PERFORM SQL-ACCESS-0096.
               PERFORM VALIDATE-STATUS-CODE-0097.
               PERFORM RECONCILE-BEDROOMS-0098.
               ...
               PERFORM NORMALISE-WITH-PROFITS-0112.
               PERFORM FORMAT-BEDROOMS-0113.
               PERFORM SQL-ACCESS-0114.
               PERFORM REFRESH-BEDROOMS-0115.
               PERFORM NORMALISE-VALUE-0116.
               ...
               PERFORM AUDIT-BEDROOMS-0125.
               PERFORM SQL-ACCESS-0126.
               EXEC CICS RETURN END-EXEC.
      *----------------------------------------------------------------*
       CALL-ZAR01D2Y-001.
               CALL 'ZAR01D2Y' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZAR01D2Y FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       ...
       SQL-ACCESS-0021.
               EXEC SQL
                     ...
                     SELECT POLICYNUMBER, PAYMENT
                       FROM GENAAR.POLICY A
                       JOIN GENAAR.CUSTOMER B
      ...
      *----------------------------------------------------------------*
       SQL-ACCESS-0096.
               ...
               END-EXEC.
      *----------------------------------------------------------------*
       VALIDATE-STATUS-CODE-0097.
               INSPECT WS-KEY-CHAR REPLACING ALL SPACES BY '0'.
               IF WS-STATUS-FAILED
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       RECONCILE-BEDROOMS-0098.
               EXEC CICS ASKTIME ABSTIME(ABS-TIME)
               END-EXEC.
               EXEC CICS FORMATTIME ABSTIME(ABS-TIME)
                         MMDDYYYY(DATE1)
