      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZFR01B2N'.
      ...
      * Called module names
       01  MOD-ZNT01QTP              PIC X(8) VALUE 'ZNT01QTP'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZFR01KM5-001.
               PERFORM CALL-ZNT01QTP-002.
      ...
      *----------------------------------------------------------------*
       CALL-ZNT01QTP-002.
               CALL 'ZNT01QTP' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZNT01QTP FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCL0255P-003.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS REWRITE FILE('KSDSFR88')
                         FROM(KSDSFR88-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
