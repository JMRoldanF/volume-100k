      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRC01BUM'.
      ...
      * Called module names
       01  MOD-ZRC01QRA              PIC X(8) VALUE 'ZRC01QRA'.
      ...
      *----------------------------------------------------------------*
       CALL-ZRC01QRA-001.
               CALL 'ZRC01QRA' USING DFHCOMMAREA
                         WS-STATUS-CODE.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS DELETE FILE('KSDSRC51')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS STARTBR FILE('KSDSRC51')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSRC51')
                            INTO(KSDSRC51-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSRC51') END-EXEC.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS REWRITE FILE('KSDSRC51')
                         FROM(KSDSRC51-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
