      ******************************************************************
      * ZCR018MQ - CUSTOMER CORRESPONDENCE                             *
      ******************************************************************
      *
      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZCR018MQ'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS STARTBR FILE('KSDSCR72')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSCR72')
                            INTO(KSDSCR72-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSCR72') END-EXEC.
      *----------------------------------------------------------------*
       AUDIT-COLOUR-0004.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS REWRITE FILE('KSDSCR72')
                         FROM(KSDSCR72-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS STARTBR FILE('KSDSCR42')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSCR42')
                            INTO(KSDSCR42-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSCR42') END-EXEC.
      *----------------------------------------------------------------*
       WRITE-ERROR-MESSAGE.
