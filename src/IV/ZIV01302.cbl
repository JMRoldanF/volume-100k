      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZIV01302'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS REWRITE FILE('KSDSIV67')
                         FROM(KSDSIV67-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS STARTBR FILE('KSDSIV67')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSIV67')
                            INTO(KSDSIV67-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSIV67') END-EXEC.
      *----------------------------------------------------------------*
       REFRESH-MAKE-0010.
               EVALUATE TRUE
                  WHEN WS-PREMIUM-TOTAL < 999
