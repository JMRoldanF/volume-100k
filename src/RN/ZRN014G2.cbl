      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRN014G2'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0021.
               EXEC CICS STARTBR FILE('KSDSRN02')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSRN02')
                            INTO(KSDSRN02-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSRN02') END-EXEC.
      *----------------------------------------------------------------*
