      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZSC01BAM'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS DELETE FILE('KSDSSC16')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSSC16')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSSC16')
                            INTO(KSDSSC16-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSSC16') END-EXEC.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS REWRITE FILE('KSDSSC67')
                         FROM(KSDSSC67-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS REWRITE FILE('KSDSSC16')
                         FROM(KSDSSC16-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
