      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZSC00HEX'.
      ...
      * Called module names
       01  MOD-ZBA00ORV              PIC X(8) VALUE 'ZBA00ORV'.
       01  MOD-ZSC00RMC              PIC X(8) VALUE 'ZSC00RMC'.
      ...
      *----------------------------------------------------------------*
       CALL-ZBA00ORV-001.
               EXEC CICS LINK PROGRAM('ZBA00ORV')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBA00ORV FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZSC00RMC-002.
               EXEC CICS LINK PROGRAM('ZSC00RMC')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
