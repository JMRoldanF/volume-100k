      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZHE018Q0'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS STARTBR FILE('KSDSHE38')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSHE38')
                            INTO(KSDSHE38-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSHE38') END-EXEC.
      *----------------------------------------------------------------*
       RESOLVE-COLOUR-0016.
               UNSTRING WS-KEY-CHAR DELIMITED BY '/'
                             INTO WS-KEY-CUSTOMER
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0027.
               EXEC CICS DELETE FILE('KSDSHE38')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
