      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZHE01A2B'.
      ...
      * Called module names
       01  MOD-ZHE01KOT              PIC X(8) VALUE 'ZHE01KOT'.
      ...
      *----------------------------------------------------------------*
       CALL-ZHE01KOT-001.
               CALL 'ZHE01KOT' USING DFHCOMMAREA
                         WS-STATUS-CODE.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS STARTBR FILE('KSDSHE05')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSHE05')
                            INTO(KSDSHE05-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSHE05') END-EXEC.
      *----------------------------------------------------------------*
       APPLY-TAX-BAND-0010.
