      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZPR0130P'.
      ...
      * Called module names
       01  MOD-ZPR01SUN              PIC X(8) VALUE 'ZPR01SUN'.
      ...
      *----------------------------------------------------------------*
       CALL-ZPR01SUN-001.
               CALL 'ZPR01SUN' USING DFHCOMMAREA
                         WS-STATUS-CODE.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSPR40')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSPR40')
                            INTO(KSDSPR40-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSPR40') END-EXEC.
      *----------------------------------------------------------------*
       CHECK-EQUITIES-0007.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS DELETE FILE('KSDSPR40')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0027.
               EXEC CICS REWRITE FILE('KSDSPR40')
                         FROM(KSDSPR40-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
