      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZPR01AXZ'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS STARTBR FILE('KSDSPR07')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSPR07')
                            INTO(KSDSPR07-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSPR07') END-EXEC.
      *----------------------------------------------------------------*
       APPLY-PREMIUM-0004.
               EVALUATE TRUE
                  WHEN WS-PREMIUM-TOTAL < 999
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS REWRITE FILE('KSDSPR07')
                         FROM(KSDSPR07-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
