      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZSE014J8'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSSE53')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSSE53')
                            INTO(KSDSSE53-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSSE53') END-EXEC.
      *----------------------------------------------------------------*
       FORMAT-PREMIUM-0007.
               IF WS-KEY-CUSTOMER = ZERO
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0012.
               EXEC CICS REWRITE FILE('KSDSSE53')
                         FROM(KSDSSE53-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
