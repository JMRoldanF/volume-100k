      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMT002OZ'.
      ...
      * Called module names
       01  MOD-ZHO007M8              PIC X(8) VALUE 'ZHO007M8'.
       01  MOD-ZAG00K3K              PIC X(8) VALUE 'ZAG00K3K'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZHO007M8-001.
               PERFORM CALL-ZAG00K3K-002.
               PERFORM CALL-ZUW000LO-003.
      ...
      *----------------------------------------------------------------*
       CALL-ZHO007M8-001.
               EXEC CICS LINK PROGRAM('ZHO007M8')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZHO007M8 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZAG00K3K-002.
               EXEC CICS LINK PROGRAM('ZAG00K3K')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZAG00K3K FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZUW000LO-003.
               EXEC CICS START TRANSID('Z0LO')
                         FROM(WS-KEY-AREA)
                         LENGTH(20)
                         RESP(WS-RESP)
               END-EXEC.
      * TRANSID Z0LO is defined against ZUW000LO
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZUW000LO FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN0255N-004.
