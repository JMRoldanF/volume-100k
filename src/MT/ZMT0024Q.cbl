      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMT0024Q'.
      ...
      * Called module names
       01  MOD-ZMT00K1H              PIC X(8) VALUE 'ZMT00K1H'.
       01  MOD-ZMT0097V              PIC X(8) VALUE 'ZMT0097V'.
      ...
      *----------------------------------------------------------------*
       CALL-ZMT00K1H-001.
               EXEC CICS LINK PROGRAM('ZMT00K1H')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZMT00K1H FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZMT0097V-002.
               EXEC CICS LINK PROGRAM('ZMT0097V')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZMT0097V FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCL0255P-003.
