      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZEN005AX'.
      ...
      * Called module names
       01  MOD-ZEN0068T              PIC X(8) VALUE 'ZEN0068T'.
       01  MOD-ZCU0006S              PIC X(8) VALUE 'ZCU0006S'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZEN0068T-001.
               PERFORM CALL-ZEN0066L-002.
               PERFORM CALL-ZBI005TW-004.
      ...
      *----------------------------------------------------------------*
       CALL-ZEN0068T-001.
               EXEC CICS LINK PROGRAM('ZEN0068T')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZEN0068T FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN0066L-002.
               MOVE 'ZEN0066L' TO WS-PROGNAME-8
      ...
      *----------------------------------------------------------------*
       CALL-ZCU0006S-003.
               EXEC CICS XCTL PROGRAM('ZCU0006S')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZCU0006S FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBI005TW-004.
               EXEC CICS START TRANSID('Z5TW')
                         FROM(WS-KEY-AREA)
                         LENGTH(20)
                         RESP(WS-RESP)
               END-EXEC.
      * TRANSID Z5TW is defined against ZBI005TW
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBI005TW FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCL0255P-005.
