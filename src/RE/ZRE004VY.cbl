      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRE004VY'.
      ...
      * Called module names
       01  MOD-ZRE00DX2              PIC X(8) VALUE 'ZRE00DX2'.
       01  MOD-ZRE00FGC              PIC X(8) VALUE 'ZRE00FGC'.
       01  MOD-ZRE00D3J              PIC X(8) VALUE 'ZRE00D3J'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZRE00DX2-001.
               PERFORM CALL-ZRE00FGC-002.
               PERFORM CALL-ZRE00D3J-003.
      ...
      *----------------------------------------------------------------*
       CALL-ZRE00DX2-001.
               EXEC CICS LINK PROGRAM('ZRE00DX2')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZRE00DX2 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZRE00FGC-002.
               EXEC CICS LINK PROGRAM('ZRE00FGC')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZRE00FGC FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZRE00D3J-003.
               EXEC CICS LINK PROGRAM('ZRE00D3J')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZRE00D3J FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZAG0255S-004.
