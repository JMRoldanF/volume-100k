      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRE017XL'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS REWRITE FILE('KSDSRE72')
                         FROM(KSDSRE72-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0030.
               EXEC CICS STARTBR FILE('KSDSRE72')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSRE72')
                            INTO(KSDSRE72-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSRE72') END-EXEC.
      *----------------------------------------------------------------*
