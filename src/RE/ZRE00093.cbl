      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRE00093'.
      ...
      * Called module names
       01  MOD-ZRE00D7R              PIC X(8) VALUE 'ZRE00D7R'.
       01  MOD-ZRE00IEP              PIC X(8) VALUE 'ZRE00IEP'.
       01  MOD-ZCL005XH              PIC X(8) VALUE 'ZCL005XH'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZRE00D7R-001.
               PERFORM CALL-ZRE00IEP-002.
               PERFORM CALL-ZCL005XH-003.
      ...
      *----------------------------------------------------------------*
       CALL-ZRE00D7R-001.
               EXEC CICS LINK PROGRAM('ZRE00D7R')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZRE00D7R FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZRE00IEP-002.
               EXEC CICS LINK PROGRAM('ZRE00IEP')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZRE00IEP FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCL005XH-003.
               EXEC CICS XCTL PROGRAM('ZCL005XH')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZCL005XH FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZMT0255L-004.
