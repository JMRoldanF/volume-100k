      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZLB0019D'.
      ...
      * Called module names
       01  MOD-ZLB00BZO              PIC X(8) VALUE 'ZLB00BZO'.
       01  MOD-ZLB00GXB              PIC X(8) VALUE 'ZLB00GXB'.
       01  MOD-ZLB00EFJ              PIC X(8) VALUE 'ZLB00EFJ'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZLB00BZO-001.
               PERFORM CALL-ZLB00GXB-002.
      ...
      *----------------------------------------------------------------*
       CALL-ZLB00BZO-001.
               EXEC CICS LINK PROGRAM('ZLB00BZO')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZLB00BZO FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZLB00GXB-002.
               EXEC CICS LINK PROGRAM('ZLB00GXB')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZLB00GXB FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZLB00EFJ-003.
               EXEC CICS LINK PROGRAM('ZLB00EFJ')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZLB00EFJ FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN0255N-004.
