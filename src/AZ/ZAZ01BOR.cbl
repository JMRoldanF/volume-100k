      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZAZ01BOR'.
      ...
      * Called module names
       01  MOD-ZCL01S2D              PIC X(8) VALUE 'ZCL01S2D'.
      ...
      *----------------------------------------------------------------*
       CALL-ZCL01S2D-001.
               CALL 'ZCL01S2D' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZCL01S2D FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN0255N-002.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSAZ26')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSAZ26')
                            INTO(KSDSAZ26-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSAZ26') END-EXEC.
      *----------------------------------------------------------------*
       REFRESH-VALUE-0007.
               EVALUATE TRUE
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0015.
               EXEC CICS REWRITE FILE('KSDSAZ26')
                         FROM(KSDSAZ26-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS DELETE FILE('KSDSAZ26')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
