      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZAN01AY9'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS STARTBR FILE('KSDSAN02')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSAN02')
                            INTO(KSDSAN02-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSAN02') END-EXEC.
      *----------------------------------------------------------------*
