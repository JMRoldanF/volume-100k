      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZFL001MM'.
      ...
      * Called module names
       01  MOD-ZQU00FJD              PIC X(8) VALUE 'ZQU00FJD'.
       01  MOD-ZBA00GHV              PIC X(8) VALUE 'ZBA00GHV'.
       01  MOD-ZBA004EG              PIC X(8) VALUE 'ZBA004EG'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZQU00FJD-001.
               PERFORM CALL-ZBA00GHV-002.
               PERFORM CALL-ZBA004EG-003.
      ...
      *----------------------------------------------------------------*
       CALL-ZQU00FJD-001.
               EXEC CICS LINK PROGRAM('ZQU00FJD')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZQU00FJD FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBA00GHV-002.
               EXEC CICS LINK PROGRAM('ZBA00GHV')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBA00GHV FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBA004EG-003.
               EXEC CICS XCTL PROGRAM('ZBA004EG')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBA004EG FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZAG0255S-004.
