      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZBI0164A'.
      ...
      * Called module names
       ...
       01  MOD-ZMT01F69              PIC X(8) VALUE 'ZMT01F69'.
      ...
      *----------------------------------------------------------------*
       CALL-ZMT01F69-002.
               CALL 'ZMT01F69' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZMT01F69 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS STARTBR FILE('KSDSBI25')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSBI25')
                            INTO(KSDSBI25-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSBI25') END-EXEC.
      *----------------------------------------------------------------*
