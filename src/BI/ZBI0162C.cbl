      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZBI0162C'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZBI01G36-001.
               PERFORM CALL-ZBI01PNC-002.
      ...
      *----------------------------------------------------------------*
       CALL-ZBI01PNC-002.
               MOVE 'ZBI01PNC' TO WS-SUBNAME-7
               CALL WS-SUBNAME-7 USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBI01PNC FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN0255N-003.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS STARTBR FILE('KSDSBI73')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSBI73')
                            INTO(KSDSBI73-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSBI73') END-EXEC.
      *----------------------------------------------------------------*
       RESOLVE-COLOUR-0004.
