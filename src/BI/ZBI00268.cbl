      ...
      * Called module names
       01  MOD-ZBI00GAA              PIC X(8) VALUE 'ZBI00GAA'.
       01  MOD-ZBI00HPY              PIC X(8) VALUE 'ZBI00HPY'.
       01  MOD-ZBI00EIY              PIC X(8) VALUE 'ZBI00EIY'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZBI00GAA-001.
               PERFORM CALL-ZBI00HPY-002.
               PERFORM CALL-ZBI00EIY-003.
      ...
      *----------------------------------------------------------------*
       CALL-ZBI00GAA-001.
               EXEC CICS LINK PROGRAM('ZBI00GAA')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBI00GAA FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBI00HPY-002.
               EXEC CICS LINK PROGRAM('ZBI00HPY')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBI00HPY FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBI00EIY-003.
               EXEC CICS LINK PROGRAM('ZBI00EIY')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZBI00EIY FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBI0255Q-004.
