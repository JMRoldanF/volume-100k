      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZIF019T2'.
      ...
      * Called module names
       01  MOD-ZIF01JCB              PIC X(8) VALUE 'ZIF01JCB'.
      ...
      *----------------------------------------------------------------*
       CALL-ZIF01JCB-001.
               CALL 'ZIF01JCB' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZIF01JCB FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCU0255O-002.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0003.
               EXEC CICS REWRITE FILE('KSDSIF41')
                         FROM(KSDSIF41-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS DELETE FILE('KSDSIF41')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               EVALUATE WS-RESP
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS STARTBR FILE('KSDSIF41')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSIF41')
                            INTO(KSDSIF41-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSIF41') END-EXEC.
      *----------------------------------------------------------------*
