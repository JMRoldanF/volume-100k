      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZPE001VY'.
      ...
      * Called module names
       01  MOD-ZPE00H7A              PIC X(8) VALUE 'ZPE00H7A'.
       01  MOD-ZPE00H4M              PIC X(8) VALUE 'ZPE00H4M'.
       01  MOD-ZPE00F5P              PIC X(8) VALUE 'ZPE00F5P'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZPE00H7A-001.
               PERFORM CALL-ZPE00H4M-002.
               PERFORM CALL-ZPE00F5P-003.
               PERFORM CALL-ZPE00423-004.
      ...
      *----------------------------------------------------------------*
       CALL-ZPE00H7A-001.
               EXEC CICS LINK PROGRAM('ZPE00H7A')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZPE00H7A FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZPE00H4M-002.
               EXEC CICS LINK PROGRAM('ZPE00H4M')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
      ...
      *----------------------------------------------------------------*
       CALL-ZPE00F5P-003.
               EXEC CICS LINK PROGRAM('ZPE00F5P')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZPE00F5P FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZPE00423-004.
