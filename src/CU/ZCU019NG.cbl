      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZCU019NG'.
      ...
      * Called module names
       01  MOD-ZCU01EX6              PIC X(8) VALUE 'ZCU01EX6'.
      ...
      *----------------------------------------------------------------*
       CALL-ZCU01EX6-001.
               CALL 'ZCU01EX6' USING DFHCOMMAREA
                         WS-STATUS-CODE.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0018.
               EXEC CICS STARTBR FILE('KSDSCU68')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSCU68')
                            INTO(KSDSCU68-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSCU68') END-EXEC.
      *----------------------------------------------------------------*
