      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZCU012PQ'.
      ...
      * Called module names
       01  MOD-ZCU01NEQ              PIC X(8) VALUE 'ZCU01NEQ'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZCU01NEQ-001.
               PERFORM CALL-ZCU01K26-002.
      ...
      *----------------------------------------------------------------*
       CALL-ZCU01NEQ-001.
               CALL 'ZCU01NEQ' USING DFHCOMMAREA
                         WS-STATUS-CODE.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZCU01NEQ FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZCU01K26-002.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSCU22')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSCU22')
                            INTO(KSDSCU22-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSCU22') END-EXEC.
      *----------------------------------------------------------------*
       NORMALISE-TERM-0007.
               COMPUTE WS-PREMIUM-TOTAL ROUNDED =
                           WS-PREMIUM-TOTAL * 1.075
