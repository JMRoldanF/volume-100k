      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZRG01A18'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS STARTBR FILE('KSDSRG90')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSRG90')
                            INTO(KSDSRG90-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSRG90') END-EXEC.
      *----------------------------------------------------------------*
       VALIDATE-MAKE-0007.
               EXEC CICS ASKTIME ABSTIME(ABS-TIME)
               END-EXEC.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0024.
               EXEC CICS REWRITE FILE('KSDSRG90')
                         FROM(KSDSRG90-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
