      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZQU016ES'.
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0006.
               EXEC CICS REWRITE FILE('KSDSQU98')
                         FROM(KSDSQU98-REC)
                         LENGTH(WS-FILE-LEN)
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
      ...
      *----------------------------------------------------------------*
       FILE-ACCESS-0009.
               EXEC CICS STARTBR FILE('KSDSQU98')
                         RIDFLD(WS-KEY-AREA)
                         RESP(WS-RESP)
               END-EXEC.
               PERFORM UNTIL WS-RESP NOT = DFHRESP(NORMAL)
                  EXEC CICS READNEXT FILE('KSDSQU98')
                            INTO(KSDSQU98-REC)
                            RIDFLD(WS-KEY-AREA)
                            RESP(WS-RESP)
                  END-EXEC
               END-PERFORM.
               EXEC CICS ENDBR FILE('KSDSQU98') END-EXEC.
      *----------------------------------------------------------------*
       FORMAT-PREMIUM-0010.
               MOVE SPACES TO WS-KEY-CHAR.
               STRING WS-KEY-CUSTOMER DELIMITED BY SIZE
